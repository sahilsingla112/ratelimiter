...
public class PreReWriteFilter extends ZuulFilter {
	private static final Logger LOG = LoggerFactory.getLogger(PreReWriteFilter.class);
	...
	public Object run() {
		...
		try {
			String url= UriComponentsBuilder.fromHttpUrl("http://localhost:8910/").path("/api").build().toUriString();
